...
const ParamItem = ({ label, value, explanation }: { label: string; value: React.ReactNode; explanation: string }) => {
  return (
    <li className="flex justify-between items-center border-b border-[#473a87]/5 pb-2 mb-2 last:border-0 last:pb-0 last:mb-0 relative group">
      <span
        className="text-[#473a87]/60 flex items-center cursor-help group-hover:text-[#473a87] transition-colors"
        title=""
      >
        {label}
      ...
      <span className="bg-white px-2.5 py-0.5 rounded-lg border border-slate-100 shadow-sm font-bold text-[#473a87] z-10">{value}</span>
...
export default function CourtExplorer({ nivsterMode = false }: CourtExplorerProps) {
  ...
  // Per-court pool membership toggle state (local mock)
  const [poolState, setPoolState] = useState<Record<string, boolean>>(
    Object.fromEntries(Object.entries(mockUserCourtData).map(([id, d]) => [id, d.inPool]))
  );
  ...
  const userCourtData = activeCourt ? mockUserCourtData[activeCourt.id] : null;
  const isInPool = activeCourt ? (poolState[activeCourt.id] ?? false) : false;

  return (
    <div className="space-y-4 animate-in fade-in slide-in-from-bottom-4 duration-500 h-[calc(100vh-14rem)] flex flex-col justify-center">
      <div className="mb-2">
        <h2 className="text-3xl font-black mb-2">Court Explorer</h2>
        <p className="text-[#473a87]/60 font-medium">Browse active jurisdictions and their specific operational rulesets.</p>
      </div>

      {/* Category Tabs */}
      <div className="flex space-x-2 border-b border-[#473a87]/10 pb-2">
        {categories.map(cat => (
          <button
            key={cat}
            onClick={() => { setActiveCourtCategory(cat); setSelectedCourtId(null); }}
            className={`px-6 py-2 text-sm font-bold rounded-full transition-all ${activeCategory === cat ? 'bg-[#473a87] text-white shadow-md' : 'text-[#473a87]/60 hover:text-[#473a87] hover:bg-[#473a87]/5'}`}
          >
            {cat}
          </button>
        ))}
      ...
      <div className="flex flex-1 overflow-hidden border border-[#473a87]/10 rounded-2xl bg-white shadow-sm">
        {/* Sidebar */}
        <div className="w-1/3 border-r border-[#473a87]/10 bg-slate-50 overflow-y-auto">
          <div className="p-4 border-b border-[#473a87]/5 text-xs font-bold text-[#473a87]/40 uppercase tracking-widest bg-slate-100/50 sticky top-0">
            {activeCategory} Courts
          </div>
          {courtsInCategory.map(court => {
            const inPool = nivsterMode && poolState[court.id];
            return (
              <button
                key={court.id}
                onClick={() => setSelectedCourtId(court.id)}
                className={`w-full text-left p-5 border-b border-[#473a87]/5 hover:bg-white transition-colors flex flex-col ${activeCourt?.id === court.id ? 'bg-white border-l-4 border-l-[#473a87]' : 'border-l-4 border-l-transparent'}`}
              >
        ...
        {/* Detail Panel */}
        <div className="w-2/3 bg-slate-50/50 flex flex-col overflow-hidden">
          {activeCourt ? (
            <div className="p-6 animate-in fade-in flex-1 flex flex-col justify-between overflow-hidden">
              <div className="mb-2">
                ...
                <div className="flex items-center space-x-3 mb-2">
                  <h2 className="text-2xl font-black">{activeCourt.metadata.name}</h2>
              ...
              {nivsterMode && userCourtData && (
                <div className="mb-4 bg-white border border-[#473a87]/10 rounded-2xl p-4 shadow-sm">
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <button
                        onClick={() => setPoolState(prev => ({ ...prev, [activeCourt.id]: !prev[activeCourt.id] }))}
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    <div className="bg-slate-50 rounded-xl p-3 text-center">
                      ...
                      <p className="text-base font-black text-purple-700">{userCourtData.stake} <span className="text-[10px]">NVR</span></p>
                    </div>
                    <div className="bg-slate-50 rounded-xl p-3 text-center">
                      <p className="text-[9px] font-black uppercase tracking-wider text-[#473a87]/40 mb-1">Unclaimed</p>
                      <p className="text-base font-black text-blue-600">{userCourtData.unclaimedSui} <span className="text-[10px]">SUI</span></p>
                    </div>
                    <div className="bg-slate-50 rounded-xl p-3 text-center">
                      <p className="text-[9px] font-black uppercase tracking-wider text-[#473a87]/40 mb-1">Locked</p>
                      <p className="text-base font-black text-amber-600">{userCourtData.lockedNvr} <span className="text-[10px]">NVR</span></p>
                    </div>
                  </div>
                </div>
              )}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {/* Timetable Panel */}
                <div className="bg-white border border-[#473a87]/10 rounded-2xl p-4 shadow-sm">
                  <h5 className="text-[10px] uppercase tracking-widest font-black text-[#473a87]/50 mb-3 border-b border-[#473a87]/10 pb-2">Timetable Parameters</h5>
                ...
                {/* Economics Panel */}
                <div className="bg-white border border-[#473a87]/10 rounded-2xl p-4 shadow-sm">
                  <h5 className="text-[10px] uppercase tracking-widest font-black text-[#473a87]/50 mb-3 border-b border-[#473a87]/10 pb-2">Economics Rules</h5>
                  <ul className="text-xs font-semibold text-[#473a87]">
